...
class QNetwork(Model):
    def __init__(self, n_actions, n_features, n_lstm_state, n_lstm_step, n_l1, n_lstm):
        ...
        self.n_l1 = n_l1
        self.n_lstm = n_lstm # We'll use this for the number of filters
        ...
        # 1. Conv1D Layer (replaces GRU/LSTM)
        #    This will slide a "window" (kernel_size) over the time steps
        self.conv1 = layers.Conv1D(filters=self.n_lstm, kernel_size=3, activation='relu', name="Conv1D_Layer")
        
        # 2. Global Pooling Layer
        #    This will reduce the time-step dimension to a single vector
        #    by averaging the features detected by the Conv1D layer.
        self.pool1 = layers.GlobalAveragePooling1D(name="GlobalPool_Layer")
        # --- End Conv1D Block ---
    ...
    def call(self, inputs):
        ...
        # Process Stream 2 (Historical State with Conv1D)
        # lstm_s_input shape: (batch, n_lstm_step, n_lstm_state)
        conv_out = self.conv1(lstm_s_input)
        # conv_out shape: (batch, n_lstm_step - 2, n_lstm) (assuming kernel_size=3)
        conv_out = self.pool1(conv_out)
        # conv_out shape: (batch, n_lstm)

        # Merge Streams
        # x shape: (batch, n_l1 + n_lstm)
        x = self.concat([s, conv_out])
