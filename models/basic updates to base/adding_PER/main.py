...
def calculate_immediate_penalty(env, ue_index, action, observation, lstm_history):
    ...
    # Penalty 2: Offloading to a congested server
        ...
        if current_congestion > (env.n_ue / env.n_edge): 
            penalty -= 0.2 * current_congestion # Penalize based on congestion level
...
def train(ue_RL_list, NUM_EPISODE):
    ...
    for episode in range(NUM_EPISODE):
        ...
        for time_index in range(env.n_time):
            ...
            for ue_index in range(env.n_ue):
                tmp_dict = {'observation': np.zeros(env.n_features),
                            ...
                            'immediate_penalty': 0.0} # <-- ADD THIS
        ...
        while True:
            ...
            time_now_action = env.time_count # Get time before env.step()
            ...
            for ue_index in range(env.n_ue):
                ...
                if observation[0] != 0: # Only if a task is arriving
                    penalty = calculate_immediate_penalty(
                        env, 
                        ue_index, 
                        action_all[ue_index], 
                        observation, 
                        current_lstm_histories[ue_index]
                    )
            ...
            for ue_index in range(env.n_ue):
                ...
                if len(update_index) != 0:
                    for update_ii in range(len(update_index)):
                        ...
                        # --- STEP 2: Add immediate penalty to final reward ---
                        ...
                        # Get stored immediate penalty from when the task arrived
                        immediate_penalty = history[time_index][ue_index]['immediate_penalty']
                        
                        # Combine rewards
                        reward = final_reward + immediate_penalty
                        # --- END STEP 2 ---
                        ...
                        ue_RL_list[ue_index].store_transition(
                            ...
                            reward, # <-- Use the combined reward
                            history[time_index][ue_index]['observation_'],
                            history[time_index][ue_index]['lstm_history_'])

                        # Store for plotting
                        ue_RL_list[ue_index].do_store_reward(episode, time_index, reward) # Store combined reward for plotting
