...
class QNetwork(Model):
    def __init__(self, n_actions, n_features, n_lstm_state, n_lstm_step, n_l1, n_lstm):
        ...
        self.dense_s = layers.Dense(self.n_l1, activation="relu", name="Dense_S")

        # --- Stream 2: Historical State (lstm_s) ---
        # Input shape: (None, n_lstm_step, n_lstm_state)
        # 1. GRU Layer (replaces LSTM, more lightweight)
        #    return_sequences=True is CRITICAL for the Attention layer
        self.gru = layers.GRU(self.n_lstm, return_sequences=True, name="GRU_Layer")
        
        # 2. Attention Layer
        #    This layer will learn to create a weighted average of the GRU outputs
        #    It takes [query, value] as input.
        #    Query = current state (s)
        #    Value = gru outputs (history)
        self.attention = layers.Attention(name="Attention_Layer")

        # --- Merged Stream ---
        # We concatenate the processed current state (s) and the context vector (history)
        self.concat = layers.Concatenate(axis=1, name="Concat")
        
        # Shared dense layer
        self.l1 = layers.Dense(self.n_l1, activation="relu", name="L1_Shared")

        # --- Dueling DQN Heads ---
        # 1. Value Stream
        self.V_dense = layers.Dense(self.n_l1, activation="relu", name="V_Dense")
        self.V_output = layers.Dense(1, name="V_Output")
        
        # 2. Advantage Stream
        self.A_dense = layers.Dense(self.n_l1, activation="relu", name="A_Dense")
        self.A_output = layers.Dense(self.n_actions, name="A_Output")

    def call(self, inputs):
        # inputs = [s_input, lstm_s_input]
        s_input = inputs[0]
        lstm_s_input = inputs[1]

        # Process Stream 1 (Current State)
        # s_input shape: (batch, n_features)
        s = self.dense_s(s_input)
        # s shape: (batch, n_l1)

        # Process Stream 2 (Historical State)
        # lstm_s_input shape: (batch, n_lstm_step, n_lstm_state)
        gru_out = self.gru(lstm_s_input)
        # gru_out shape: (batch, n_lstm_step, n_lstm)
        
        # Apply Attention
        # We use the current state 's' as the "query" to ask the "history" (gru_out)
        # what is most important.
        # Query shape must be (batch, 1, n_l1)
        # Value shape must be (batch, n_lstm_step, n_lstm)
        query = tf.expand_dims(s, axis=1)
        
        # context_vector shape: (batch, 1, n_lstm)
        context_vector = self.attention(inputs=[query, gru_out])
        
        # Squeeze to (batch, n_lstm) to match 's' shape for concatenation
        context_vector = tf.squeeze(context_vector, axis=1)

        # Merge Streams
        # x shape: (batch, n_l1 + n_lstm)
        x = self.concat([s, context_vector])
        ...
        A = self.A_dense(x)
        A = self.A_output(A) # Shape: (batch, n_actions)
        
        # Combine V and A
        # Q = V(s) + (A(s,a) - mean(A(s,a)))
        Q = V + (A - tf.reduce_mean(A, axis=1, keepdims=True))
        
        return Q, A, V # Return Q-values

# --- End GRU + Attention Model ---
